    ...
    <div className="circular-gallery__track">
      {cards.map((item, index) => {
        ...
        >
          ...
          <span className="circular-gallery__number">0{index + 1}</span>
          <span className="circular-gallery__copy"><small>{item.en}</small><strong>{item.text}</strong><i>VIEW PROJECT</i></span>
        </button>
      })}
    </div>
  </div>
